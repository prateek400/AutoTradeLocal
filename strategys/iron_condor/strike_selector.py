import math
from config import get_instrument_params
from market_data import get_ltp_of_symbol

def round_to_nearest(x, base):
    return int(round(x / base) * base)

def get_expected_move(iv, days_to_expiry, spot_price):
    """
...
def is_iv_acceptable(iv, min_iv=10, max_iv=20):
    return min_iv <= iv <= max_iv

def get_strikes_for_instrument(iv, days_to_expiry, instrument):
    params = get_instrument_params(instrument)
    instrument_name = params["exchange"] + ":" + params["trading_symbol"]
    min_iv = params["min_iv"]
    max_iv = params["max_iv"]
    strike_step = params["strike_step"]
    spread_width = params["spread_width"]
    return get_strike_prices(get_ltp_of_symbol(instrument_name), iv, days_to_expiry, strike_step, spread_width, min_iv, max_iv)

def get_strike_prices(spot_price, iv, days_to_expiry, strike_step=50, spread_width=100, min_iv=10, max_iv=20):
    """
    ...
    """
    if not is_iv_acceptable(iv, min_iv, max_iv):
        print(f"[IV Filter] Skipping trade: IV={iv} outside range {min_iv}-{max_iv}")
        return None

    expected_move = get_expected_move(iv, days_to_expiry, spot_price)
    expected_move_rounded = round_to_nearest(expected_move, strike_step)
    spot_rounded = round_to_nearest(spot_price, strike_step)
